...
#define I2C_CONSET_I2EN			((0x40)) /*!< I2C interface enable */
...
//	static const TypeId::I2cMaster{{ id }}Scl Scl;
	...
	initialize(uint32_t target_clock = 100000)
	{
		...
		I2Cx->CONCLR = (I2C_CONCLR_AAC | I2C_CONCLR_STAC | I2C_CONCLR_I2ENC);
	...
	reset(DetachCause cause=DetachCause::SoftwareReset) {
		readBytesLeft = 0;
		writeBytesLeft = 0;
		if (xpcc::accessor::asVolatile(delegate)) delegate->stopped(cause);
		...
		case I2C_I2STAT_M_TX_DAT_ACK:
			...
			if (writeBytesLeft != 0) {
				...
				case xpcc::I2c::Operation::Restart:
					...
					I2Cx->CONCLR = I2C_CONCLR_AAC | I2C_CONCLR_SIC;
...
private:
	...
	static inline uint8_t i2read() {
		return (I2Cx->DAT & I2C_I2DAT_BITMASK);
	}
