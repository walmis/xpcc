...
{
	LPC_SC->PCONP |= (1<<10); //enable PCSSP bit
...
// ----------------------------------------------------------------------------
bool
xpcc::lpc::SpiMaster1::setBuffer(uint16_t length, uint8_t* transmit, uint8_t* receive, BufferIncrease bufferIncrease)
{
	if (!isFinished()) {
		return false;
	}
...
bool
xpcc::lpc::SpiMaster1::transfer(TransferOptions options)
{
...
bool
xpcc::lpc::SpiMaster1::transferSync(TransferOptions options)
{
	return transfer(options);
}

bool
xpcc::lpc::SpiMaster1::isFinished()
{
