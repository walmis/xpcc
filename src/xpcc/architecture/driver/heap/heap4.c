...
void *pvReturn = NULL;
	...
	//vTaskSuspendAll();
	{
		...
		if( ( xWantedSize > 0 ) && ( xWantedSize < HEAP_SIZE ) )
		{
...
unsigned char *pucHeapEnd;
	...
	//configASSERT( ( ( ( unsigned long ) &__heap_start ) & ( ( unsigned long ) portBYTE_ALIGNMENT_MASK ) ) == 0UL );

	/* xStart is used to hold a pointer to the first item in the list of free
	blocks.  The void cast is used to prevent compiler warnings. */
	xStart.pxNextFreeBlock = ( void * ) &__heap_start;
	xStart.xBlockSize = ( size_t ) 0;

	/* pxEnd is used to mark the end of the list of free blocks and is inserted
	at the end of the heap space. */
	pucHeapEnd = &__heap_start + HEAP_SIZE;
	...
	pxFirstFreeBlock = ( void * ) &__heap_start;
	pxFirstFreeBlock->xBlockSize = HEAP_SIZE - heapSTRUCT_SIZE;
