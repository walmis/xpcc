...
static const uchar Sbox[256] = { // forward s-box
		0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b,
				0xfe, 0xd7, 0xab, 0x76, 0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59,
				0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0,
				0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5,
				0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15, 0x04, 0xc7, 0x23, 0xc3,
				0x18, 0x96, 0x05, 0x9a, 0x07, 0x12, 0x80, 0xe2, 0xeb, 0x27,
				0xb2, 0x75, 0x09, 0x83, 0x2c, 0x1a, 0x1b, 0x6e, 0x5a, 0xa0,
				0x52, 0x3b, 0xd6, 0xb3, 0x29, 0xe3, 0x2f, 0x84, 0x53, 0xd1,
...
static const uchar InvSbox[256] = { // inverse s-box
		 };

// combined Xtimes2[Sbox[]]
static const uchar Xtime2Sbox[256] = { 0xc6, 0xf8, 0xee, 0xf6, 0xff, 0xd6, 0xde,
...
static const uchar Xtime3Sbox[256] = { 0xa5, 0x84, 0x99, 0x8d, 0x0d, 0xbd, 0xb1,
		...
		0xb8, 0xc3, 0xb0, 0x77, 0x11, 0xcb, 0xfc, 0xd6, 0x3a };
...
static const uchar Xtime2[256] = { 0x00, 0x02, 0x04, 0x06, 0x08, 0x0a, 0x0c,
...
static const uchar Xtime9[256] = { 0x00, 0x09, 0x12, 0x1b, 0x24, 0x2d, 0x36,
...
static const uchar XtimeB[256] = { 0x00, 0x0b, 0x16, 0x1d, 0x2c, 0x27, 0x3a,
...
static const uchar XtimeD[256] = { 0x00, 0x0d, 0x1a, 0x17, 0x34, 0x39, 0x2e,
...
static const uchar XtimeE[256] = { 0x00, 0x0e, 0x1c, 0x12, 0x38, 0x36, 0x24,
...
FASTCODE static void ShiftRows(uchar *state) {
	uchar tmp;
...
FASTCODE static void InvShiftRows(uchar *state) {
	uchar tmp;
	...
	state[7] = InvSbox[state[11]], state[11] = InvSbox[state[15]], state[15] =
			tmp;
}

// recombine and mix each row in a column
FASTCODE static void MixSubColumns(uchar *state) {
	uchar tmp[4 * Nb];
...
FASTCODE static void InvMixSubColumns(uchar *state) {
	uchar tmp[4 * Nb];
	int i;
...
static void AddRoundKey(unsigned *state, const unsigned *key) {
	int idx;
...
void aes128_init(const uchar *key, aes128_ctx_t* ctx) {
	uchar tmp0, tmp1, tmp2, tmp3, tmp4;
	unsigned idx;

	memcpy(ctx->expkey, key, Nk * 4);

	for (idx = Nk; idx < Nb * (Nr + 1); idx++) {
		tmp0 = ctx->expkey[4 * idx - 4];
		tmp1 = ctx->expkey[4 * idx - 3];
		tmp2 = ctx->expkey[4 * idx - 2];
		tmp3 = ctx->expkey[4 * idx - 1];
		if (!(idx % Nk)) {
			tmp4 = tmp3;
			tmp3 = Sbox[tmp0];
		...
		ctx->expkey[4 * idx + 0] = ctx->expkey[4 * idx - 4 * Nk + 0] ^ tmp0;
		ctx->expkey[4 * idx + 1] = ctx->expkey[4 * idx - 4 * Nk + 1] ^ tmp1;
		ctx->expkey[4 * idx + 2] = ctx->expkey[4 * idx - 4 * Nk + 2] ^ tmp2;
		ctx->expkey[4 * idx + 3] = ctx->expkey[4 * idx - 4 * Nk + 3] ^ tmp3;
	}
}

// encrypt one 128 bit block
void aes128_enc(uchar *state, aes128_ctx_t* ctx) {

	unsigned round;

	//memcpy (state, in, Nb * 4);
	AddRoundKey((unsigned *) state, (unsigned *) ctx->expkey);
	...
	for (round = 1; round < Nr + 1; round++) {
		...
		AddRoundKey((unsigned *) state, (unsigned *) ctx->expkey + round * Nb);
	}

	//memcpy (out, state, sizeof(state));
}

void aes128_dec(uchar *block, aes128_ctx_t* ctx) {

	unsigned round;

	//memcpy (state, in, sizeof(state));

	AddRoundKey((unsigned *) block, (unsigned *) ctx->expkey + Nr * Nb);
	InvShiftRows(block);

	for (round = Nr; round--;) {
		AddRoundKey((unsigned *) block, (unsigned *) ctx->expkey + round * Nb);
		if (round)
